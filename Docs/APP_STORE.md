...
**Capture on a Pro Max.** App Store Connect requires the 6.9" set (1320×2868);
a 6.3" device produces 1206×2622, which the upload form rejects.

```bash
Scripts/screenshots.sh "iPhone 17 Pro Max"
```

That captures nine states and writes them to `Artifacts/screenshots/`. Other
sizes are optional and are scaled from the 6.9" set.
